...
Generate a random sample of valid ConvexAdam parameter combinations for
hyper-parameter search, excluding combinations known to crash from the
...
For each "avoid" file in ``AVOID_PATHS``, the script:
  ...
  5. Saves the selected combinations and the constant grid parameters to
     ``convex_adam_sweep_<dataset>_ch<channels>_nc<N>.json``, along with a
     bar chart of the per-key value distribution as a sibling ``.png``.
"""
...
SEARCH_SPACE_PATH = "config/registration/defaults/original_convex_adam_search.json"
...
for avoid_path in AVOID_PATHS:
    ...
    output_path = (
        ...
        f"convex_adam_sweep_{dataset_name}_ch{num_channels}_nc{NUM_COMBINATIONS}.json"
    )
